...
FROM {{ ref('stg_fator_complexidade_ust') }} F
LEFT JOIN {{ ref('stg_cargo') }} C ON C.ID = F.CARGO_ID
LEFT JOIN {{ ref('stg_nivel') }} N ON N.ID = F.NIVEL_ID
LEFT JOIN {{ ref('stg_contrato') }} CT ON CT.ID = F.CONTRATO_ID
