...
FROM {{ ref('stg_ab_user') }} U
LEFT JOIN {{ ref('stg_cargo') }} C ON C.ID = U.CARGO_ID
LEFT JOIN {{ ref('stg_nivel') }} N ON N.ID = U.NIVEL_ID
LEFT JOIN {{ ref('stg_coordenacao') }} CO ON CO.ID = U.COORDENACAO_ID
