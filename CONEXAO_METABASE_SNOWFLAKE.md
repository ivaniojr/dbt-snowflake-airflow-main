...
| File path | `/metabase-data/rsa_key.p8` |
| Warehouse | `DRAGON_WH` |
| Database name (case sensitive) | `DRAGON_DB` |
...
- **Não** usar senha: o toggle "Authenticate with user and password" deve permanecer desligado, pois o usuário `DRAGON` está configurado para autenticação por chave RSA (key-pair), igual ao dbt.
- O campo **File path** aponta para dentro do container do Metabase, não para o seu computador. O arquivo `src/dbt/rsa_key.p8` já é montado automaticamente em `/metabase-data/rsa_key.p8` (somente leitura) pelo serviço `metabase` definido em [airflow/docker-compose.yaml](airflow/docker-compose.yaml).
